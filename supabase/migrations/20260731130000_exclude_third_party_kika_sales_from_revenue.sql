ALTER TABLE public.transactions
  ADD COLUMN IF NOT EXISTS include_in_revenue boolean NOT NULL DEFAULT true;

COMMENT ON COLUMN public.transactions.include_in_revenue IS
  'Whether the transaction belongs to Daily Flowers revenue reporting. False keeps third-party proceeds visible for reconciliation without counting them as revenue or VAT turnover.';

UPDATE public.transactions
SET include_in_revenue = false
WHERE channel = 'bold_afs'
  AND paid_at >= '2026-03-01 00:00:00 Europe/Amsterdam'::timestamptz
  AND paid_at < '2026-04-01 00:00:00 Europe/Amsterdam'::timestamptz
  AND vat_rate = 0
  AND article_number IN ('168', '169', '170')
  AND lower(trim(coalesce(product_name, ''))) = 'mollie x kika';
...
COMMENT ON VIEW public.vw_monthly_revenue_actuals IS
  'Monthly actual revenue. Transactions marked include_in_revenue=false remain available for reconciliation but are excluded from revenue and VAT reporting. Shopify cancelled orders and fully refunded orders with zero current total are zeroed; other Shopify orders use current totals. Mollie Facturen uses issued non-cancelled sales invoices, and WeFact Facturen excludes omzethuur, facilitair and energie categories.';
