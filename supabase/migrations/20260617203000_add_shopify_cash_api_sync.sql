ALTER TABLE public.shopify_cash_sessions
  ...
  ADD COLUMN IF NOT EXISTS expected_closing_balance numeric(14,2),
  ADD COLUMN IF NOT EXISTS total_cash_sales numeric(14,2),
  ADD COLUMN IF NOT EXISTS total_cash_refunds numeric(14,2),
  ADD COLUMN IF NOT EXISTS net_cash_sales numeric(14,2),
  ADD COLUMN IF NOT EXISTS total_adjustments numeric(14,2),
...
BEGIN
  IF NOT EXISTS (
    SELECT 1
    FROM pg_constraint
    WHERE conname = 'shopify_cash_sessions_shopify_session_unique'
      AND conrelid = 'public.shopify_cash_sessions'::regclass
  ) THEN
    ALTER TABLE public.shopify_cash_sessions
      ADD CONSTRAINT shopify_cash_sessions_shopify_session_unique
      UNIQUE (shop_domain, shopify_session_id);
  END IF;
END $$;

CREATE INDEX IF NOT EXISTS idx_shopify_cash_sessions_shopify_session
  ON public.shopify_cash_sessions (shop_domain, shopify_session_id)
  WHERE shopify_session_id IS NOT NULL;

CREATE TABLE IF NOT EXISTS public.shopify_cash_session_transactions (
  ...
  CONSTRAINT shopify_cash_session_transactions_unique UNIQUE (shop_domain, shopify_transaction_id)
);

CREATE INDEX IF NOT EXISTS idx_shopify_cash_session_transactions_session
  ON public.shopify_cash_session_transactions (shop_domain, shopify_session_id);

CREATE INDEX IF NOT EXISTS idx_shopify_cash_session_transactions_order
  ON public.shopify_cash_session_transactions (order_id)
  WHERE order_id IS NOT NULL;

CREATE INDEX IF NOT EXISTS idx_shopify_cash_session_transactions_processed
  ON public.shopify_cash_session_transactions (processed_at DESC);

DROP TRIGGER IF EXISTS trg_shopify_cash_session_transactions_updated_at
  ON public.shopify_cash_session_transactions;
CREATE TRIGGER trg_shopify_cash_session_transactions_updated_at
  BEFORE UPDATE ON public.shopify_cash_session_transactions
  FOR EACH ROW EXECUTE FUNCTION public.tg_set_updated_at();

GRANT SELECT, INSERT, UPDATE, DELETE ON public.shopify_cash_session_transactions TO authenticated;
GRANT ALL ON public.shopify_cash_session_transactions TO service_role;

ALTER TABLE public.shopify_cash_session_transactions ENABLE ROW LEVEL SECURITY;

DROP POLICY IF EXISTS "shopify_cash_session_transactions_select_auth"
  ON public.shopify_cash_session_transactions;
CREATE POLICY "shopify_cash_session_transactions_select_auth"
  ON public.shopify_cash_session_transactions FOR SELECT TO authenticated USING (true);

DROP POLICY IF EXISTS "shopify_cash_session_transactions_all_auth"
  ON public.shopify_cash_session_transactions;
CREATE POLICY "shopify_cash_session_transactions_all_auth"
  ON public.shopify_cash_session_transactions FOR ALL TO authenticated
...
cash_tx_by_order AS (
  SELECT
    ct.order_id AS external_id,
    max(ct.order_name) AS order_name,
    max(ct.shop_domain) AS shop_domain,
    max(ct.shopify_session_id) AS shopify_session_id,
    count(*) FILTER (WHERE lower(coalesce(ct.status, '')) = 'success')::int AS cash_transaction_count,
    coalesce(
      sum(
        CASE
      ...
      0
    )::numeric(14,2) AS cash_transaction_amount
...
pos_orders AS (
  SELECT
    ...
    coalesce(ct.cash_transaction_count, 0)::int AS cash_transaction_count,
    coalesce(ct.cash_transaction_amount, 0)::numeric(14,2) AS shopify_cash_transaction_amount,
    (
    ...
    )::numeric(14,2) AS fallback_cash_amount,
    CASE
      WHEN coalesce(ct.cash_transaction_count, 0) > 0 THEN coalesce(ct.cash_transaction_amount, 0)
      ELSE (
        coalesce(os.current_total_price, os.net_payment, os.line_discounted_total, os.total_price, 0)
        - coalesce(p.shopify_payment_amount, 0)
      )
    END::numeric(14,2) AS cash_amount,
    CASE
      WHEN coalesce(ct.cash_transaction_count, 0) > 0 THEN 'shopify_cash_transactions'
      ELSE 'inferred_from_order_minus_shopify_payments'
    END AS cash_source,
    ct.shop_domain AS cash_shop_domain,
    ct.shopify_session_id AS cash_shopify_session_id
  FROM public.shopify_order_summaries os
  LEFT JOIN provider_by_order p ON p.external_id = os.external_id
  LEFT JOIN cash_tx_by_order ct
    ON ct.external_id = os.external_id
    OR (ct.external_id IS NULL AND ct.order_name = os.order_name)
...
cash_sessions AS (
  SELECT s.*
  FROM public.shopify_cash_sessions s
  WHERE s.import_source = 'shopify_cash_api'
     OR NOT EXISTS (
       SELECT 1
       FROM public.shopify_cash_sessions api
       WHERE api.import_source = 'shopify_cash_api'
         AND api.location_id = s.location_id
         AND abs(extract(epoch FROM (api.session_start - s.session_start))) < 60
     )
),
matched AS (
  SELECT
    po.*,
    coalesce(sd.id, st.id) AS cash_session_id,
    coalesce(sd.location_id, st.location_id) AS location_id,
    coalesce(sd.location_name, st.location_name) AS location_name,
    coalesce(sd.register_id, st.register_id) AS register_id,
    coalesce(sd.session_start, st.session_start) AS session_start,
    coalesce(sd.session_end, st.session_end) AS session_end,
    coalesce(sd.status, st.status) AS session_status,
    row_number() OVER (
      PARTITION BY po.order_summary_id
      ORDER BY
        CASE WHEN sd.id IS NOT NULL THEN 0 ELSE 1 END,
        st.session_start DESC NULLS LAST
    ) AS rn
  FROM pos_orders po
  LEFT JOIN cash_sessions sd
    ON po.cash_shop_domain = sd.shop_domain
   AND po.cash_shopify_session_id = sd.shopify_session_id
  LEFT JOIN cash_sessions st
    ON sd.id IS NULL
   AND po.processed_at >= st.session_start
   AND (st.session_end IS NULL OR po.processed_at <= st.session_end)
)
SELECT
  order_summary_id,
  external_id,
...
WITH order_daily AS (
  SELECT
    ...
    coalesce(sum(shopify_cash_transaction_amount), 0)::numeric(14,2) AS shopify_cash_transaction_amount,
    count(*) FILTER (WHERE cash_transaction_count > 0)::int AS shopify_cash_transaction_order_count,
...
SELECT
  ...
  coalesce(od.shopify_cash_transaction_amount, 0)::numeric(14,2) AS shopify_cash_transaction_amount,
  coalesce(od.shopify_cash_transaction_order_count, 0)::int AS shopify_cash_transaction_order_count,
...
SELECT
  ...
  sum(shopify_cash_transaction_amount)::numeric(14,2) AS shopify_cash_transaction_amount,
  sum(shopify_cash_transaction_order_count)::int AS shopify_cash_transaction_order_count,
