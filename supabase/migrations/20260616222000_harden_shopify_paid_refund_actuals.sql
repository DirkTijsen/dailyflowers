ALTER TYPE public.tx_status ADD VALUE IF NOT EXISTS 'partially_paid';
...
WITH shopify_base AS (
  SELECT
    ...
    channel,
    COALESCE(current_total_price, 0) AS current_gross,
    CASE
...
shopify_actuals AS (
  SELECT
    ...
    COALESCE(sum(current_gross), 0) AS gross_total,
    COALESCE(sum(current_gross - current_vat), 0) AS net_total,
...
COMMENT ON VIEW public.vw_monthly_revenue_actuals IS
  'Monthly actual revenue. Shopify includes only paid, partially_refunded and refunded orders; current totals are after refunds.';
