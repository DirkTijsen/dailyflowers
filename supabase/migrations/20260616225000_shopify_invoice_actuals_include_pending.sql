...
WITH shopify_base AS (
  SELECT
    ...
    channel,
    COALESCE(total_price, current_total_price, 0) AS invoice_gross,
    COALESCE(total_tax, current_total_tax, line_tax_total, 0) AS invoice_vat
...
COMMENT ON VIEW public.vw_monthly_revenue_actuals IS
  'Monthly actual revenue. Shopify includes every order summary with VAT invoice data, including pending orders, and uses the invoice basis.';
