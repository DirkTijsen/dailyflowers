...
WITH shopify_base AS (
  SELECT
    ...
    channel,
    CASE
      WHEN raw_payload->>'imported_from' = 'shopify_csv'
        THEN COALESCE(total_price, current_total_price, 0)
      ELSE COALESCE(total_price, current_total_price, 0)
    END AS invoice_gross,
    CASE
      WHEN raw_payload->>'imported_from' = 'shopify_csv'
        THEN COALESCE(total_tax, current_total_tax, line_tax_total, 0)
      ELSE COALESCE(total_tax, current_total_tax, line_tax_total, 0)
    END AS invoice_vat
...
COMMENT ON VIEW public.vw_monthly_revenue_actuals IS
  'Monthly actual revenue. Shopify includes only paid/refunded order summaries with VAT invoice data and uses the VAT invoice basis instead of pending/current open order values.';
